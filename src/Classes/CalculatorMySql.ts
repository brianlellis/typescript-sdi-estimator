...
export default class CalculatorMySql {
  ...
  validType( data_type: string ): number {
    switch( data_type ) {
    case 'tinyint':
    case 'smallint':
    case 'mediumint':
    ...
    case 'float':
    case 'double':
    case 'decimal': return this.numericSizes( data_type );
    case 'char':
    case 'varchar':
    case 'tinytext':
    case 'mediumtext':
    ...
    case 'blob':
    case 'mediumblob':
    case 'longblob':  return this.stringSizes( data_type );
    case 'datetime':
    case 'datetime2':
    case 'smalldatetime':
    case 'date':
    case 'time': return this.dateSize( data_type );
    }
    return 0;
  }

  numericSizes( data_type: string ): number {
  ...
  stringSizes( data_type: string , char_count?: number ): number {
    ...
    case 'mediumtext':
    case 'longtext':
    case 'text':        return char_count ?? 0; // this.byteSize( char_count );
    case 'blob':
    case 'mediumblob':
  ...
  dateSize( data_type: string ): number {
    switch( data_type ) {
  ...
  fivePercentEstimatePad( value: number ): number {
